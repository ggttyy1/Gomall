...
func chargeHandler(ctx context.Context, handler interface{}, arg, result interface{}) error {
	...
	case *streaming.Args:
		st := s.Stream
		req := new(payment.PaymentServiceReq)
...
func (p *ChargeArgs) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetReq() {
		p.Req = new(payment.PaymentServiceReq)
	}
...
func (p *ChargeArgs) Unmarshal(in []byte) error {
	msg := new(payment.PaymentServiceReq)
...
var ChargeArgs_Req_DEFAULT *payment.PaymentServiceReq

func (p *ChargeArgs) GetReq() *payment.PaymentServiceReq {
	if !p.IsSetReq() {
...
type ChargeResult struct {
	Success *payment.PaymentServiceResp
}

var ChargeResult_Success_DEFAULT *payment.PaymentServiceResp

func (p *ChargeResult) FastRead(buf []byte, _type int8, number int32) (n int, err error) {
	if !p.IsSetSuccess() {
		p.Success = new(payment.PaymentServiceResp)
	}
...
func (p *ChargeResult) Unmarshal(in []byte) error {
	msg := new(payment.PaymentServiceResp)
	if err := proto.Unmarshal(in, msg); err != nil {
		return err
	}
	p.Success = msg
...
func (p *ChargeResult) GetSuccess() *payment.PaymentServiceResp {
	if !p.IsSetSuccess() {
...
func (p *ChargeResult) SetSuccess(x interface{}) {
	p.Success = x.(*payment.PaymentServiceResp)
}
...
func (p *kClient) Charge(ctx context.Context, Req *payment.PaymentServiceReq) (r *payment.PaymentServiceResp, err error) {
